...
/* ============================================================
   ...
   ============================================================ */
const SUPABASE_URL = "https://dnkofdwufldtzimnpico.supabase.co";      // e.g. "https://abcd1234.supabase.co"
...
async function sbAuthError(r, fallback) {
  let body = {};
  try { body = await r.json(); } catch {}
  console.error("Supabase auth error:", r.status, body);
  if (body.error_code === "over_email_send_rate_limit") {
    throw new Error("Email rate limit reached. Wait an hour or raise the limit in Supabase → Authentication → Rate Limits.");
  }
  if (body.error_code === "signup_disabled") {
    throw new Error("Sign-ups are disabled in Supabase. Enable Email provider or invite users from the dashboard.");
  }
  if (r.status === 500 && /email/i.test(body.msg || "")) {
    const id = body.error_id ? ` Log ID: ${body.error_id}` : "";
    throw new Error(
      `Email could not be sent — SMTP misconfiguration in Supabase.${id} ` +
      "Check Authentication → SMTP: Resend host smtp.resend.com, port 587, user resend, sender on a verified domain. " +
      "See Resend → Domains and Supabase → Authentication → Logs."
    );
  }
...
async function sbMagicLink(email) {
  ...
  if (!r.ok) await sbAuthError(r, "Could not send the link. Check the email address.");
}
